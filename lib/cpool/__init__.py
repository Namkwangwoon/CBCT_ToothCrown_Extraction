...
import top_pool, bottom_pool, left_pool, right_pool, front_pool, back_pool
...
class FrontPoolFunction(Function):
    @staticmethod
    def forward(ctx, input):
        output = front_pool.forward(input)[0]
        ctx.save_for_backward(input)
        return output
    ...
    def backward(ctx, grad_output):
        input  = ctx.saved_variables[0]
        output = front_pool.backward(input, grad_output)[0]
        return output

class BackPoolFunction(Function):
    @staticmethod
    def forward(ctx, input):
        output = back_pool.forward(input)[0]
        ctx.save_for_backward(input)
        return output
    ...
    def backward(ctx, grad_output):
        input  = ctx.saved_variables[0]
        output = back_pool.backward(input, grad_output)[0]
        return output
...
class FrontPool(nn.Module):
    def forward(self, x):
        return FrontPoolFunction.apply(x)

class BackPool(nn.Module):
    def forward(self, x):
        return BackPoolFunction.apply(x)
