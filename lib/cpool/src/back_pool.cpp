...
std::vector<torch::Tensor> top_pool_forward(
    ...
    torch::Tensor input_temp  = input.select(2, depth - 1);
    torch::Tensor output_temp = output.select(2, depth - 1);
    output_temp.copy_(input_temp);

    torch::Tensor max_temp;
    for (int64_t ind = 1; ind < depth; ++ind) {
        input_temp  = input.select(2, depth - ind - 1);
        output_temp = output.select(2, depth - ind);
        max_temp    = output.select(2, depth - ind - 1);
...
std::vector<torch::Tensor> top_pool_backward(
    ...
    auto input_temp = input.select(2, depth - 1);
    max_val.copy_(input_temp);

    max_ind.fill_(depth - 1);

    auto output_temp      = output.select(2, depth - 1);
    auto grad_output_temp = grad_output.select(2, depth - 1);
    ...
    for (int32_t ind = 1; ind < depth; ++ind) {
        input_temp = input.select(2, depth - ind - 1);
        torch::gt_out(gt_mask, input_temp, max_val);

        torch::masked_select_out(max_temp, input_temp, gt_mask);
        max_val.masked_scatter_(gt_mask, max_temp);
        max_ind.masked_fill_(gt_mask, depth - ind - 1);

        grad_output_temp = grad_output.select(2, depth - ind - 1).unsqueeze(2);
