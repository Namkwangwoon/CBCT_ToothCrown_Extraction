...
DATA_PATH = os.path.abspath('../datasets/[20230623]osstem_dataset')
...
# dir_names = ['5차/서대홍']
# dir_names = ['6차/82_01']
dir_names = ['6차/case03', '7차/case08/mask']
...
for dir_name in dir_names:
    ...
    for idx, file_name in enumerate(file_names):
        ...
        np_gt = np_gt.reshape((496, 800, 800))

        # nii_data = nib.Nifti1Image(np_data, affine=np.eye(4))
        nii_gt = nib.Nifti1Image(np_gt, affine=np.eye(4))
        nib.save(nii_gt, os.path.join(DATA_PATH, dir_name, file_name[:-9]+'.nii.gz'))
